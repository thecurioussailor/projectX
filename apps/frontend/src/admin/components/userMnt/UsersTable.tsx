...
import UserDetailSidePop from "./UserDetailSidePop";
                ...
                <div className="overflow-x-scroll lg:overflow-x-hidden">
                    <table className="w-full text-left min-w-max lg:min-w-full">
                        ...
                        <tr className="border-t border-gray-200 text-[#1B3155]">
                            ...
                            <th className="lg:w-2/12 px-4">Email</th>
                            <th className="lg:w-2/12 px-4">Username</th>
                            <th className="lg:w-2/12 px-4">Is Banned</th>
                            <th className="lg:w-2/12 px-4">Created At</th>
...
const UsersTableRow = ({user, index}: {user: User, index: number}) => {
    ...
    return (
        <tr className="border-t border-gray-200 h-20 text-[#1B3155]">
            ...
            <td className="font-semibold px-4">{user.name}</td>
            <td className="px-4">{user.email}</td>
            <td className="px-4">{user.username}</td>
            <td className="px-4">{user.isBanned ? "Yes" : "No"}</td>
            <td className="px-4">{new Date(user.createdAt).toLocaleDateString("en-US", { month: "long", day: "numeric" })}</td>
            <td className="px-4">
                <div 
                    ref={ref}
                    className="relative"
                >
                    ...
                    {showMenu && (
                        <div className="absolute z-10 -top-20 right-4 border rounded-3xl bg-white overflow-clip">
                            <button     
                    ...
                    {isSidePopOpen && (
                        <UserDetailSidePop user={user} onClose={() => setIsSidePopOpen(false)} />
                    )}
                </div>
            </td>
        </tr>
