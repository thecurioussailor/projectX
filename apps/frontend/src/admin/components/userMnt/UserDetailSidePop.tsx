...
const UserDetailSidePop = ({ user, onClose }: { user: User, onClose: () => void }) => {
      ...
      >
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-[#7e37d8]">User Detail</h1>
            <button 
              onClick={handleClose}
          ...
          <div ref={printRef} className="space-y-4">
            <div className="border-b pb-3 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">Location:</p>
                    <p className="font-medium text-sm">{user.location}</p>
                </div>
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">Is Banned:</p>
                    <p className="font-medium text-sm">{user.isBanned ? "Yes" : "No"}</p>
                </div>
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">Created At:</p>
                    <p className="font-medium text-sm">{new Date(user.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>
                </div>
            </div>
          </div>
        </div>
...
export default UserDetailSidePop;