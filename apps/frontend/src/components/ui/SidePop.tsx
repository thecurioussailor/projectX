...
const SidePop = ({ sale, onClose }: { sale: Sale, onClose: () => void }) => {
      ...
      >
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-[#7e37d8]">Order Details</h1>
            <button 
              onClick={handleClose}
          ...
          <div className="space-y-4">
            ...
            <div className="border-b pb-3 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <p className="font-medium text-sm">{sale.status}</p>
                </div>
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">Product Type:</p>
                    <p className="font-medium text-sm">{sale.productType}</p>
                </div>
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">Created At:</p>
                    <p className="font-medium text-sm">{new Date(sale.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>
                </div>
            </div>
            <div className="border-b pb-3 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">User ID:</p>
                    <p className="font-medium text-sm">{sale.user.id}</p>
                </div>
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">Username:</p>
                    <p className="font-medium text-sm">{sale.user.username}</p>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <p className="font-medium text-sm">{sale.user.email}</p>
                </div>
                <div className="flex items-center gap-2">
                    <p className="text-xs text-[#718096]">Phone:</p>
                    <p className="font-medium text-sm">{sale.user.phone}</p>
                </div>
            </div>
            <div className="border-b pb-3">
                {sale.productType === 'DIGITAL_PRODUCT' && (
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="font-medium text-sm">{sale.digitalProduct?.title}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <p className="text-xs text-[#718096]">Product ID:</p>
                            <p className="font-medium text-sm">{sale.digitalProduct?.id}</p>
                        </div>
                    </div>
                )}
                {sale.productType === 'TELEGRAM_PLAN' && (
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                            <p className="text-xs text-[#718096]">Telegram Plan:</p>
                            <p className="font-medium text-sm">{sale.telegramPlan?.name}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <p className="text-xs text-[#718096]">Product ID:</p>
                            <p className="font-medium text-sm">{sale.telegramPlan?.id}</p>
                        </div>
                    </div>
                )}
            </div>
